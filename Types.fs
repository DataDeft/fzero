...
type Node = {
  ...
  HiddenState: Option<bool>
  Reward: uint32
} with
  member this.Expanded (node : Node) : bool =
    match node.Children with
    | Some _ -> true
    | _ -> false

  member this.Value (node : Node) : float =
    match node.VisitCount with
    | zero when zero = uint32(0) -> 0.0
    | _ -> float(node.ValueSum) / float(node.VisitCount)
